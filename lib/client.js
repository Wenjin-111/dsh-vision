// dsh-vision browser half: the 设置 > 插件 > 插件配置 card that edits
// the `dsh-vision` settings section owned by the host half. Self-contained
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function installStyles() {
      ...
      tag.dataset.pluginCss = 'dsh-vision/plugin-card'
      tag.textContent = CSS
    ...
    function apply(ctx) {
      const scope = ctx.settingsScope.bind({ namespace: 'dsh-vision' })
      ...
      ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'dsh-vision: card locale')
      ...
      ctx.effect(installStyles, 'dsh-vision: card styles')
      ctx.effect(
        () =>
